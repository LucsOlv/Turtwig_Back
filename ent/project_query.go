...
type ProjectQuery struct {
	...
	withCodeEmbeddings  *CodeEmbeddingQuery
	withHistories       *HistoryQuery
	withHistoryProjects *HistoryProjectQuery
...
// QueryCodeEmbeddings chains the current query on the "code_embeddings" edge.
func (pq *ProjectQuery) QueryCodeEmbeddings() *CodeEmbeddingQuery {
	query := (&CodeEmbeddingClient{config: pq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := pq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (pq *ProjectQuery) QueryHistories() *HistoryQuery {
	query := (&HistoryClient{config: pq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := pq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// QueryHistoryProjects chains the current query on the "history_projects" edge.
func (pq *ProjectQuery) QueryHistoryProjects() *HistoryProjectQuery {
	query := (&HistoryProjectClient{config: pq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := pq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (pq *ProjectQuery) Clone() *ProjectQuery {
	...
	return &ProjectQuery{
		...
		withCodeEmbeddings:  pq.withCodeEmbeddings.Clone(),
		withHistories:       pq.withHistories.Clone(),
		withHistoryProjects: pq.withHistoryProjects.Clone(),
...
// WithCodeEmbeddings tells the query-builder to eager-load the nodes that are connected to
// the "code_embeddings" edge. The optional arguments are used to configure the query builder of the edge.
func (pq *ProjectQuery) WithCodeEmbeddings(opts ...func(*CodeEmbeddingQuery)) *ProjectQuery {
	query := (&CodeEmbeddingClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	pq.withCodeEmbeddings = query
	return pq
}
...
func (pq *ProjectQuery) WithHistories(opts ...func(*HistoryQuery)) *ProjectQuery {
	query := (&HistoryClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	pq.withHistories = query
	return pq
}

// WithHistoryProjects tells the query-builder to eager-load the nodes that are connected to
// the "history_projects" edge. The optional arguments are used to configure the query builder of the edge.
func (pq *ProjectQuery) WithHistoryProjects(opts ...func(*HistoryProjectQuery)) *ProjectQuery {
	query := (&HistoryProjectClient{config: pq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	pq.withHistoryProjects = query
	return pq
}
...
func (pq *ProjectQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Project, error) {
	var (
		...
		loadedTypes = [5]bool{
			pq.withOwner != nil,
			pq.withMembers != nil,
			pq.withCodeEmbeddings != nil,
			pq.withHistories != nil,
			pq.withHistoryProjects != nil,
		}
	)
	...
	if query := pq.withCodeEmbeddings; query != nil {
		if err := pq.loadCodeEmbeddings(ctx, query, nodes,
			func(n *Project) { n.Edges.CodeEmbeddings = []*CodeEmbedding{} },
			func(n *Project, e *CodeEmbedding) { n.Edges.CodeEmbeddings = append(n.Edges.CodeEmbeddings, e) }); err != nil {
			return nil, err
		}
	}
	if query := pq.withHistories; query != nil {
		if err := pq.loadHistories(ctx, query, nodes,
			func(n *Project) { n.Edges.Histories = []*History{} },
			func(n *Project, e *History) { n.Edges.Histories = append(n.Edges.Histories, e) }); err != nil {
			return nil, err
		}
	}
	if query := pq.withHistoryProjects; query != nil {
		if err := pq.loadHistoryProjects(ctx, query, nodes,
			func(n *Project) { n.Edges.HistoryProjects = []*HistoryProject{} },
			func(n *Project, e *HistoryProject) { n.Edges.HistoryProjects = append(n.Edges.HistoryProjects, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (pq *ProjectQuery) loadCodeEmbeddings(ctx context.Context, query *CodeEmbeddingQuery, nodes []*Project, init func(*Project), assign func(*Project, *CodeEmbedding)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Project)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(codeembedding.FieldProjectID)
	}
	query.Where(predicate.CodeEmbedding(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(project.CodeEmbeddingsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (pq *ProjectQuery) loadHistories(ctx context.Context, query *HistoryQuery, nodes []*Project, init func(*Project), assign func(*Project, *History)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Project)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(history.FieldProjectID)
	}
	query.Where(predicate.History(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(project.HistoriesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (pq *ProjectQuery) loadHistoryProjects(ctx context.Context, query *HistoryProjectQuery, nodes []*Project, init func(*Project), assign func(*Project, *HistoryProject)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Project)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(historyproject.FieldProjectID)
	}
	query.Where(predicate.HistoryProject(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(project.HistoryProjectsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
