...
// The HistoryProjectFunc type is an adapter to allow the use of ordinary
// function as HistoryProject mutator.
type HistoryProjectFunc func(context.Context, *ent.HistoryProjectMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f HistoryProjectFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.HistoryProjectMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.HistoryProjectMutation", m)
}
