...
func (hq *HistoryQuery) QueryProject() *ProjectQuery {
	query := (&ProjectClient{config: hq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := hq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (hq *HistoryQuery) QueryHistoryProjects() *HistoryProjectQuery {
	query := (&HistoryProjectClient{config: hq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := hq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (hq *HistoryQuery) FirstIDX(ctx context.Context) uuid.UUID {
...
func (hq *HistoryQuery) OnlyIDX(ctx context.Context) uuid.UUID {
...
func (hq *HistoryQuery) IDs(ctx context.Context) (ids []uuid.UUID, err error) {
...
func (hq *HistoryQuery) Clone() *HistoryQuery {
	...
	return &HistoryQuery{
		...
		withProject:         hq.withProject.Clone(),
		withHistoryProjects: hq.withHistoryProjects.Clone(),
...
func (hq *HistoryQuery) WithProject(opts ...func(*ProjectQuery)) *HistoryQuery {
	query := (&ProjectClient{config: hq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	hq.withProject = query
	return hq
}
...
func (hq *HistoryQuery) WithHistoryProjects(opts ...func(*HistoryProjectQuery)) *HistoryQuery {
	query := (&HistoryProjectClient{config: hq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	hq.withHistoryProjects = query
	return hq
}
...
func (hq *HistoryQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*History, error) {
	...
	if query := hq.withProject; query != nil {
		if err := hq.loadProject(ctx, query, nodes, nil,
			func(n *History, e *Project) { n.Edges.Project = e }); err != nil {
			return nil, err
		}
	}
	if query := hq.withHistoryProjects; query != nil {
		if err := hq.loadHistoryProjects(ctx, query, nodes,
			func(n *History) { n.Edges.HistoryProjects = []*HistoryProject{} },
			func(n *History, e *HistoryProject) { n.Edges.HistoryProjects = append(n.Edges.HistoryProjects, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (hq *HistoryQuery) loadProject(ctx context.Context, query *ProjectQuery, nodes []*History, init func(*History), assign func(*History, *Project)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*History)
	for i := range nodes {
...
func (hq *HistoryQuery) loadHistoryProjects(ctx context.Context, query *HistoryProjectQuery, nodes []*History, init func(*History), assign func(*History, *HistoryProject)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(historyproject.FieldHistoryID)
	}
	query.Where(predicate.HistoryProject(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(history.HistoryProjectsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "history_id" returned %v for node %v`, fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (hq *HistoryQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := hq.ctx.Fields; len(fields) > 0 {
		...
		if hq.withProject != nil {
			_spec.Node.AddColumnOnce(history.FieldProjectID)
		}
	}
